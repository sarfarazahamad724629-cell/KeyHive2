...
export default function SubkeysPage({ ctx }) {
  const { subkeys, api, loadSubkeys, loadMasterKeys, masterKeys, notify, fmtNum, fmtDate, quotaColor, modal, setModal, setRevealedToken, revealedToken, copyText } = ctx;
  ...
  return <div className='page active'><div style={{ padding: '32px 36px' }}><div className='page-header' style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}><div><div className='page-title'>Subkeys</div><div className='page-sub'>Scoped API tokens you distribute to employees, clients, or teams</div></div><button className='btn btn-primary' onClick={() => setModal('createsubkey')}>+ Create subkey</button></div>
    <div className='card' style={{ padding: 0 }}><div className='table-wrap'><table><thead><tr><th>Name</th><th>Token</th><th>Provider</th><th>Quota</th><th>Max req</th><th>Expires</th><th>Status</th><th>Actions</th></tr></thead><tbody>{!subkeys.length ? <tr><td colSpan='8' style={{ textAlign: 'center', color: 'var(--dim)', padding: '32px' }}>No subkeys yet — create one above</td></tr> : subkeys.map((sk) => {const pct = Math.min(100, Math.round((sk.tokens_used / sk.monthly_token_limit) * 100)); const col = quotaColor(sk.tokens_used, sk.monthly_token_limit); const rawToken = sk.token || sk.token_prefix || ''; const masked = rawToken ? `${rawToken.slice(0, 12)}••••••••${rawToken.slice(-4)}` : 'hidden'; return <tr key={sk.id}><td style={{ fontWeight: 500 }}>{sk.name}</td><td><div className='token-box' style={{ maxWidth: '200px' }}><span className='token-val'>{masked}</span><button className='token-copy' onClick={() => copyText(sk.token || sk.token_prefix || '')}>copy</button></div></td><td><span style={{ fontSize: '12px', background: 'var(--bg3)', padding: '3px 8px', borderRadius: '4px', fontFamily: 'DM Mono, monospace' }}>{sk.provider}</span></td><td style={{ minWidth: '120px' }}><div className='quota-bar'><div className={`quota-fill ${col}`} style={{ width: `${pct}%` }} /></div><div className='quota-text'>{fmtNum(sk.tokens_used)} / {fmtNum(sk.monthly_token_limit)}</div></td><td className='mono'>{fmtNum(sk.request_count || 0)} / {fmtNum(sk.max_requests || 5000)}</td><td style={{ fontSize: '12px', color: 'var(--muted)' }}>{fmtDate(sk.expires_at)}</td><td><span className={`badge ${sk.status}`}>{sk.status}</span></td><td style={{display:'flex',gap:'6px'}}><button className='btn btn-sm btn-ghost' onClick={()=>api('/api/subkeys/'+sk.id,{method:'PATCH',body:{status:'paused'}}).then(loadSubkeys)}>Pause</button><button className='btn btn-sm btn-green' onClick={()=>api('/api/subkeys/'+sk.id,{method:'PATCH',body:{status:'active'}}).then(loadSubkeys)}>Activate</button><button className='btn btn-sm btn-danger' onClick={()=>api('/api/subkeys/'+sk.id,{method:'PATCH',body:{status:'revoked'}}).then(loadSubkeys)}>Revoke</button></td></tr>;})}</tbody></table></div></div>
